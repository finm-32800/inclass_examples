...
Provides easy access to paths and dates used in the project.
...
This is a trimmed version of the settings module used in
`pydoit/04_jupytext_notebooks`. The order of precedence for a variable is:

1. Command line arguments (e.g., ``--DATA_DIR=/path/to/data``)
2. Environment variables (including those in a ``.env`` file)
...
def find_all_caps_cli_vars(argv=sys.argv):
    ...
    with a long option, for example, --DATA_DIR or --OUTPUT_DIR.
    When that option is found, the value of the option is returned.

    For example, if the command line is:
    ```
    python settings.py --DATA_DIR=/path/to/data --OUTPUT_DIR=/path/to/output
    ```
...
## Dates
# GDP starts in 1947. END_DATE is dynamic on purpose: this pipeline is
# re-run daily by GitHub Actions, and each run should pull data through today.
defaults["START_DATE"] = pd.to_datetime("1947-01-01")
defaults["END_DATE"] = pd.Timestamp.today().normalize()
...
def if_relative_make_abs(path):
    ...
    PosixPath('/Users/jdoe/GitRepositories/blank_project/_data')
    ```
    """
    path = Path(path)
    if path.is_absolute():
